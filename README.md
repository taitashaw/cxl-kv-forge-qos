...
## Phase 2.2 signoff status

| metric | value |
|---|---|
| Reference part | xczu7ev-ffvc1156-2-e (largest license-eligible MPSoC) |
| Closed-timing clock | **400 MHz** (2.500 ns) |
| Post-route WNS (setup) | **+0.033 ns (MET)** |
| Post-route TNS / failing endpoints | **0.000 ns / 0 of 77,540** |
| Hold WNS / failing | **+0.010 ns / 0** |
| Datapath width (AXIS request/response) | 256 bits |
| Sustained throughput | **256 × 400 / 1000 = 102.4 Gb/s** per AXIS stream (request + response = 204.8 Gb/s aggregate) |
| Arbitration-path latency | ~11-12 cycles ≈ **27.5 to 30.0 ns** at 400 MHz |
| Pipeline | 2-stage qmgr + registered credit→qmgr enqueue crossing + 3-stage tournament-tree arbiter with wave-admission interlock + 2-stage token bucket |
| XSim regression | 13 / 13 PASS |
| Bitstream | `results/impl/kvq_top_wrapper.bit` (19.3 MB) |

The 400 MHz close also surfaced and fixed a latent dequeue data hazard (Finding B in the audit report): the pipelined tournament arbiter could grant one un-popped queue head multiple times across overlapping waves — silently duplicating that request and losing its successor — and a wave-admission interlock now enforces exact-once grants, proven by the new backpressure-injection test.

The prior milestone, 350 MHz with WNS +0.004 ns, stands in the build progression below. Full closure narrative in `docs/architecture.md`. Final signoff doc: `results/PHASE2_2_FINAL.md`.

## Visual artifacts

![Integrated block design](docs/block_design/kvq_top_bd.png)

*Integrated block design on xczu7ev: Cortex-A53 PS (`ps_e_0`) drives two AXI SmartConnects — `smartconnect_axil` (PS HPM0_LPD → kvq_top_0 s_axi_lite) and `smartconnect_dma_ctrl` (PS HPM0_FPD → axi_dma_0 control). DMA data plane (`smartconnect_data`) bridges axi_dma_0 MM2S/S2MM streams to PS S_AXI_HP0_FPD. `clk_wiz_0` multiplies PS PL_CLK0 (100 MHz) up to the design clock — 400 MHz as of Phase 2.2; the PNG predates the retarget and shows the identical topology. PNG generated from the Vivado-exported PDF via pdftocairo at 150 DPI.*
...
Expected outputs: two PNGs, ~2 µs and ~4-6 µs windows of the testbench run that cover reset / first-AXIL-program / first-arrival, and the multi-tenant priority + deadline contention spike (T9 / T10 of the 13-test directed regression).
...
```bash
bash scripts/run_xsim.sh         # 13-test directed regression
...
## Phase 2 through Phase 2.2 — what changed structurally
...
| **Phase 2.2 (xczu7ev)** | **+ registered enqueue crossing, arbiter wave-admission interlock** | **400 MHz** |
